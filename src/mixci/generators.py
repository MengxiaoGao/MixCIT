...
"""
benchmark_debiased.py
=====================

Standalone benchmarking harness for the local-polynomial debiased
conditional independence test.

Compares the new debiased statistic Delta_n^(p) against the following
baselines, all implemented from scratch in this file (i.e. with no
dependence on any prior project code beyond data_generating.py):

  ours_debiased     -- New debiased test (analytic p-value)
  ours_debiased_perm-- New debiased test (permutation p-value)
  kci               -- Kernel Conditional Independence test (HSIC-based)
  fisherz           -- Fisher's Z partial-correlation test
  cmiknn            -- Kraskov / Frenzel-Pompe KNN-CMI test
  chisq             -- Pearson conditional chi-square (discrete only)
  gsq               -- Likelihood-ratio G-test (discrete only)
  ours_raw_knn      -- Raw kn-NN difference Delta_n (baseline for the
                       debiasing comparison; same neighborhoods as the
                       debiased version but with simple averaging)

Outputs (under output_dir/):
  benchmark_debiased_raw.csv      per-trial outcomes
  benchmark_debiased_summary.csv  aggregated Type I / Power / Runtime
  runtime_debiased_summary.csv    method-by-N runtime pivot

Run:
  python benchmark_debiased.py

Requires only numpy, pandas, scipy, scikit-learn, joblib, and the
data_generating.py and MixCI_debiased* modules from this project.
"""
